    ...
    def __init__(self, n_node, node_emd_init):
        ...
        with tf.variable_scope('discriminator'):
            if self.node_emd_init is not None:
                ...
                self.embedding_matrix = tf.get_variable(name="embedding",
                                                        shape=(self.n_node,config.n_emb),
        ...
        self.loss = tf.reduce_sum(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=self.label, logits=self.score)) + config.lambda_dis * (
                ...
                tf.nn.l2_loss(tf.norm(self.node_embedding - self.node_neighbor_embedding, ord=2, axis=1) 
                / tf.norm(self.node_embedding * self.node_neighbor_embedding, ord=2, axis=1)) +
