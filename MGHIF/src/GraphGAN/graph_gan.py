...
class GraphGAN(object):
    def __init__(self):
        ...
        self.n_node,self.root_nodes, self.graph = utils.read_edges(config.train_filename,config.test_filename)
        # self.root_nodes = [i for i in range(self.n_node)]
        if os.path.isfile(config.pretrain_emb_filename_d):
            ...
            self.node_embed_init_d = None
            self.node_embed_init_g = None
    ...
    # minimize parameters /theta_G and /theta_D of the social relation GAN, part of the total objective function.
    def train(self):
        ...
        for epoch in range(config.n_epochs):
            ...
            for d_epoch in range(config.n_epochs_dis):
                ...
                if d_epoch % config.dis_interval == 0:
                    center_index, neighbor_index, labels = self.prepare_data_for_d()
                # training
                ...
                for start in start_list:
                    ...
                    self.sess.run(self.discriminator.d_updates,
                                  feed_dict={self.discriminator.node_id: np.array(center_index[start:end]),
                                             self.discriminator.node_neighbor_id: np.array(neighbor_index[start:end]),
        ...
        for i in self.root_nodes:
            if np.random.rand() < config.update_ratio:
                ...
                if len(pos) != 0 and neg is not None:
                    current_index = self.root_nodes.index(i)
                    posindex = [self.root_nodes.index(node) for node in pos] if type(pos)==type([]) else []
                    negindex = [self.root_nodes.index(node) for node in neg] if type(neg)==type([]) else []
                    # positive samples
                    center_index.extend([current_index] * len(posindex))
                    neighbor_index.extend(posindex)
                    labels.extend([1] * len(posindex))

                    # negative samples
                    center_index.extend([current_index] * len(posindex))
                    neighbor_index.extend(negindex)
                    labels.extend([0] * len(negindex))
        return center_index, neighbor_index, labels
    ...
    def prepare_data_for_g(self):
        ...
        for i in range(len(node_pairs)):
            for pair in node_pairs[i]:
                index_1.append(self.root_nodes.index(pair[0]))
                index_2.append(self.root_nodes.index(pair[1]))
        reward = self.sess.run(self.discriminator.reward,
                               feed_dict={self.discriminator.node_id: np.array(index_1),
                                          self.discriminator.node_neighbor_id: np.array(index_2)})
        return index_1, index_2, reward
    ...
    def sample(self, root, tree, sample_num, for_d):
        ...
        Returns:
            samples: list, the nodes id of the sampled nodes
        ...
        while len(samples) < sample_num:
            ...
            while True:
                ...
                current_index = self.root_nodes.index(current_node)
                neighbor_index = [self.root_nodes.index(node) for node in node_neighbor]
                relevance_probability = all_score[current_index, neighbor_index]
    ...
    def write_embeddings_to_file(self):
        ...
        for i in range(2):
            ...
            index = np.array(self.root_nodes).reshape(-1, 1)
